...
class S_AES_window(ttk.Frame):
    def __init__(self, master):
        ...
        # 文本信息
        hdr_txt = "请输入密钥、明文或密文"
        hdr = ttk.Label(master=self, text=hdr_txt, width=80)
        ...
        result_txt = " "
        self.result_label = ttk.Label(master=self, text=result_txt, width=80)
    ...
    def act_encode(self):
        try:
            ...
            if plain_str == "" and key_str == "":
                raise Exception('明文或密钥未输入，请检查')
            key_list = ['无', '无', '无']
            key_index = 0
            while len(key_str) / 16 != 0:
                ...
                key_index += 1
            if key_index - 1 != self.num:
                raise Exception('密钥格式错误，请重新输入')
            if bool(re.match('^[01]*$', plain_str)) and len(plain_str) == 16:
                ciper_text_str = encode(plain_str, key_list[0])
                # 多重加密
                for i in range(self.num):
                    ciper_text_str = encode(ciper_text_str, key_list[i + 1])
                self.result_label[
                    'text'] = f"加密结果: {ciper_text_str},已复制在密文中。\nKEY1={key_list[0]}, KEY2={key_list[1]}, KEY3={key_list[2]}"
                self.cipher_text.set(ciper_text_str)
            else:
                ...
                for i in range(self.num):
                    ...
                    ciper_text_str = binary_trans_Unicode(ciper_text_binary)
                self.result_label[
                    'text'] = f"加密结果: {ciper_text_str},已复制在密文中. \nKEY1={key_list[0]}, KEY2={key_list[1]}, KEY3={key_list[2]}"  # 将结果显示在GUI界面上
                self.cipher_text.set(ciper_text_str)
        except:
    ...
    def act_decode(self):
        try:
            ...
            key_list = ['无', '无', '无']
            key_index = 0
            if cipher_str == "" and key_str == "":
                raise Exception('密文或密钥未输入，请检查')
            while len(key_str) / 16 != 0:
                key = key_str[:16]
                key_str = key_str[16:]
                key_list[key_index] = key
            ...
            if key_index - 1 != self.num:
                raise Exception('密钥格式错误，请重新输入')
            if bool(re.match('^[01]*$', cipher_str)) and len(cipher_str) == 16:
                plain_text_str = decode(cipher_str, key_list[key_index - 1])
                # 多重解密
                for i in range(self.num):
                    plain_text_str = decode(plain_text_str, key_list[key_index - i - 2])
                self.result_label[
                    'text'] = f"解密结果: {plain_text_str},已复制在明文中. \nKEY1={key_list[0]}, KEY2={key_list[1]}, KEY3={key_list[2]}"
                self.plain_text.set(plain_text_str)
            else:
...
if __name__ == '__main__':
    app = ttk.Window("S-AES", "cyborg", resizable=(False, False))
    S_AES_window(app)
    app.mainloop()
